...
complement_data <- read_csv(file.path(data_directory, "inferred_sex_chromosome_complement-2.csv"))
...
ggsave(filename = file.path(output_directory, "TCGA_male_count_stacked_bar_Y_expression.png"), 
...
for (gene in y_genes) {
  ...
  ggsave(filename = file.path(output_directory, paste0("TCGA_male_count_stacked_bar_", gene, ".png")), plot = plot)
...
# Save the plot to the output directory
ggsave(filename = file.path(output_directory, "TCGA_female_count_stacked_bar_X_expression.png"), 
       plot = male_y_expression_plot)
