...
Project Overview
This project analyzes gene expression data from The Cancer Genome Atlas (TCGA) and copy number data for sex chromosomes from the paper "Loss of chromosome Y in primary tumors" by Qi et al., 2023. The goal is to explore the relationship between gene expression and the ploidy of sex chromosomes.
...
create_gene_violin_plot_by_sex <- function(data, gene_expression, ploidy, remove_na = TRUE) {
  ...
  ggplot(data, aes(x = spaced_interaction, y = !!sym(gene_expression), fill = sex)) + 
    ...
    scale_fill_manual(values = c("female" = "#ffd241ff", "male" = "#ffec417f"), 
                      name = "Reported Sex") +
...
for (gene in gene_list) {
  ...
  ggsave(filename = file.path(output_directory, paste0("TCGA_male_", gene, "_violin_plot.pdf")), plot = male_plot)
}
...
ggsave(filename = file.path(output_directory, paste0("TCGA_female_XIST_violin_plot.pdf")), plot = female_plot)
...
for (gene in gene_list) {
  ...
  ggsave(filename = file.path(output_directory, paste0("TCGA_male_", gene, "_violin_plot_with_stats.pdf")), plot = male_plot_with_stats)
}
...
ggsave(filename = file.path(output_directory, paste0("TCGA_female_XIST_violin_plot_with_stats.pdf")), plot = female_plot_with_stats)
