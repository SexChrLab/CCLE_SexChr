...
### - LAML - Acute Myeloid Leukemia (blood)
...
### - DLBC - Lymphoid Neoplasm Diffuse Large B-cell Lymphoma (blood)
### - MESO - Mesothelioma
...
#                           "ESCA", "GBM", "HNSC", "KICH", "KIRC", "KIRP", "LIHC","LUAD","LUSC", "DLBC", "MESO", "OV", "PAAD", "PCPG", 


### List the TCGA types to run
tcga_studies_to_prep <- c("PRAD", "READ","SARC", "SKCM", "STAD", "TGCT", "THCA", "THYM", "UCS", "UCEC", "UVM")

working_path <- "C:/Users/livin/ASU FILES/Reseach Files/Genomic Sex Chromosome/"
setwd(working_path)
...
for (t in tcga_studies_to_prep) {
  ...
  age_index <- clindat[["demographic"]][["age_at_index"]]
  
  # Include sample type information
  sample_data <- cases() %>%
    GenomicDataCommons::filter(~ project.project_id == study) %>%
    GenomicDataCommons::select("samples.sample_type") %>%
    response_all()
  cases <- sample_data$results$samples
  # sample_types <- sapply(cases, function(x) x$sample_type)
  sample_types <- sapply(cases, function(x) paste(x$sample_type, collapse = ", "))
  
  # Combine these into a slimmed down dataframe of metadata
  meta_df <- data.frame(cbind(case_ids,
                              mf_list, 
                              status_list,
                              surv_times, follow_list,age_index,sample_types = format(sample_types, width = 80, justify = "left")))
  ...
  # # Filter for tumor-adjacent samples
  # tumor_adjacent_df <- meta_df %>%
  #   filter(grepl("Solid Tissue Normal", sample_types))
  # 
  # tumor_adjacent_fname <- paste(study, 'Tumor_Adjacent_META.tsv', sep = "-")
  # tumor_adjacent_path <- paste(data_out_path, tumor_adjacent_fname, sep = "")
  # write_tsv(
  #   tumor_adjacent_df,
  #   tumor_adjacent_path,
  #   na = "NA",
  ...
  #   progress = show_progress()
  # )
  # # Write the tumor_adjacent_df to a CSV file
  # write.csv(tumor_adjacent_df, "tumor_adjacent_df.csv", row.names = FALSE)
