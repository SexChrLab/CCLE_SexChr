...
# Filter male data
sex_check_m <- dplyr::filter(sex_check, annotated_sex == "male")
df_points2 <- sex_check_m %>%
  transmute(XIST_Y = paste(status_XIST, "XIST_", status_Y, "Y", sep = ""))
sex_check_m <- cbind(sex_check_m, df_points2)

# Filter female data
sex_check_f <- dplyr::filter(sex_check, annotated_sex == "female")
df_points3 <- sex_check_f %>%
  transmute(XIST_Y = paste(status_XIST, "XIST_", status_Y, "Y", sep = ""))
sex_check_f <- cbind(sex_check_f, df_points3)

# Fit Kaplan-Meier survival function estimates for males and females
...
# Plot Kaplan-Meier survival function estimate for males
ggsurvplot(
  km_m,
  data = sex_check_m,
  fun = "pct",
...
# Plot Kaplan-Meier survival function estimate for females
ggsurvplot(
  km_f,
  data = sex_check_f,
  fun = "pct",
