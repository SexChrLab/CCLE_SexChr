...
for (study in cancer_types) {
  ...
  ychr_counts <- counts[ychr_genes, , drop=FALSE]
  xchr_counts <- counts[xchr_genes, , drop=FALSE]
  
  new_counts1 <- data.frame(t(xchr_counts)) # xchr genes
  colnames(new_counts1) <- xchr_gnames
  
  new_counts2 <- data.frame(t(ychr_counts)) # ychr genes
  colnames(new_counts2) <- ychr_gnames
  
  new_counts <- cbind(new_counts1, new_counts2) # combine new_counts1 (xchr genes) and new_counts2 (ychr)
  ...
  for (x in c_uuid) {
    if (length(id_sets[[x]]) > 1) {
      ...
      repeats_df <- bind_rows(repeats_df, new_counts[rows_keep, ])
  ...
  for (i in c_uuid) {
    ...
    if (length(id_sets[[i]]) > 1) {
      ...
      for (j in seq_along(iid)) {
        sample_type <- sample_types[j]
        
        # Update sample types
        if (!is.na(sample_type) && sample_type == "Solid Tissue Normal") {
          counts_plus[iid[j], "Solid_Tissue_Normal"] <- "yes"
          sex_check[i, "Solid_Tissue_Normal"] <- "yes"
        }
        if (!is.na(sample_type) && sample_type == "Blood Derived Normal") {
          counts_plus[iid[j], "Blood_Derived_Normal"] <- "yes"
          sex_check[i, "Blood_Derived_Normal"] <- "yes"
        }
        if (!is.na(sample_type) && sample_type == "Primary Tumor") {
          counts_plus[iid[j], "Primary_Tumor"] <- "yes"
          sex_check[i, "Primary_Tumor"] <- "yes"
        }
        if (!is.na(sample_type) && sample_type == "Recurrent Tumor") {
          counts_plus[iid[j], "Recurrent_Tumor"] <- "yes"
          sex_check[i, "Recurrent_Tumor"] <- "yes"
        }
        if (!is.na(sample_type) && sample_type == "Primary Blood Derived Cancer - Peripheral Blood") {
      ...
      for (type in sample_types) {
        if (!is.na(type) && type == "Solid Tissue Normal") {
          counts_plus[iid, "Solid_Tissue_Normal"] <- "yes"
          sex_check[i, "Solid_Tissue_Normal"] <- "yes"
        }
        if (!is.na(type) && type == "Blood Derived Normal") {
          counts_plus[iid, "Blood_Derived_Normal"] <- "yes"
          sex_check[i, "Blood_Derived_Normal"] <- "yes"
        }
        if (!is.na(type) && type == "Primary Tumor") {
          counts_plus[iid, "Primary_Tumor"] <- "yes"
          sex_check[i, "Primary_Tumor"] <- "yes"
        }
        if (!is.na(type) && type == "Recurrent Tumor") {
          counts_plus[iid, "Recurrent_Tumor"] <- "yes"
          sex_check[i, "Recurrent_Tumor"] <- "yes"
        }
        if (!is.na(type) && type == "Primary Blood Derived Cancer - Peripheral Blood") {
  ...
  #     sex_check[i, "sample_types"] <- paste(sample_types, collapse=", ")
  #     
  #     if (all(new_counts[iid, c("DDX3Y", "USP9Y", "UTY", "ZFY", "RPS4Y1")] < 1.0)) {
  #       counts_plus[iid, "status_Y"] <- "no"
  #       sex_check[i, "status_Y"] <- "no"
  #     } else {
  #       counts_plus[iid, "status_Y"] <- "yes"
